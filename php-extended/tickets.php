...
function tk_notify_staff($pdo, string $title, string $body, string $link, int $excludeUserId = 0): void {
    $q = $pdo->prepare("SELECT id FROM users WHERE role IN ('admin','superadmin','moderator','support') AND id != ?");
    $q->execute([$excludeUserId]);
    foreach ($q->fetchAll() as $st) bk_notify((int)$st['id'], $title, $body, $link, 'ticket');
}
