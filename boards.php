...
if ($page === 'my-boards') {
    ...
    ?><main class="wrap page">
      ...
      <?php foreach($sales as $o):?><tr>
        ...
        <td><?php if($o['status']==='paid'):?><form method="post" style="display:flex;gap:6px;flex-wrap:wrap"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="board_ship"><input type="hidden" name="order_id" value="<?=$o['id']?>"><input class="field" style="height:34px;width:150px;padding:6px" name="tracking_code" placeholder="کد رهگیری (اختیاری)"><button class="btn btn-primary btn-sm">📦 ارسال شد</button></form><?php elseif($o['status']==='shipped'):?><span class="muted" style="font-size:11px">در انتظار تأیید خریدار</span><?php endif;?></td>
